...
# import os
# os.chdir("C:\\Users\\udi\\workspace\\ChessEtudeSolver")
# exec(compile(open("Solver.py").read(), "Solver.py", 'exec'))
...
ord_col_row_to_position = lambda ord_col, row : "{0}{1}".format(chr(ord_col), row)
...
class Piece:
	...
	def __repr__(self):
		return "Piece: {0}, position: {1}".format(piece_to_char(self), self.position)
...
def get_step_string(step, board_before_step):
	# Doesn't include:
	#  Promotion, en-passent, casteling.
...
{0}
 +-----------------+
   a b c d e f g h""".format('\n'.join(["{0}|{1} |".format(8-i, widen(line)) for i,line in enumerate(self.state)]))
...
def print_didnt_solve():
	print("Didn't solve :(")
	print("Check maybe solution involves promotion, casteling or en-passent.")
	...
	for step3 in threats:
		...
		for step2 in solutions_dict:
			if solutions_dict[step2] == step3:
				...
				print("{0}{1}... {2}".format(PRINT_SOLUTIONS_PREFIX * prefix_level,
				prefix_level, get_step_string(step2, board_before_step2)))

		assert board_after_step2 != None
		print("{0}  =>  {1}. {2}".format(PRINT_SOLUTIONS_PREFIX * (prefix_level + 1), \
